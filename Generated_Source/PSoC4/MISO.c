...
#include "MISO.h"
...
#if defined(MISO__PC)
    #define MISO_SetP4PinDriveMode(shift, mode)  \
    do { \
        MISO_PC =   (MISO_PC & \
                                (uint32)(~(uint32)(MISO_DRIVE_MODE_IND_MASK << \
                                (MISO_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (MISO_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define MISO_SetP4PinDriveMode(shift, mode)  \
        do { \
            MISO_USBIO_CTRL_REG = (MISO_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(MISO_DRIVE_MODE_IND_MASK << \
                                    (MISO_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (MISO_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		MISO_SetP4PinDriveMode(MISO__0__SHIFT, mode);
    }
#endif
...
void MISO_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  MISO_INTCFG & (uint32)(~(uint32)position);
    MISO_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
